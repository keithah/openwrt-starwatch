...
export function SettingsView({config, onSave, onTest, onRegenerate, onCopyToken, newToken, notice}) {
  if (!config) return html`<main class="view"><div class="empty">Settings unavailable.</div></main>`;
  const submit = event => {
  ...
  return html`<main class="view settings-view"><div class="view-heading"><div><span class="eyebrow">DAEMON CONFIGURATION</span><h1>Settings</h1></div><a href="#/">← Dashboard</a></div>${notice&&html`<div class="notice">${notice}</div>`}<form onSubmit=${submit}>
